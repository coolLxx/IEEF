...
def get_config():
    ...
    parser.add_argument("-rl_dial_one_epoch", type=int, default=200, help='一个epoch用多少个对话')
...
class InteractionEnvironment(object):
    ...
    def generate_single_dialog_t5(self, user_goal, with_logprob=False, agent=None):
        ...
        while is_continue(dial_gen):  # 需要判断一个会话是否结束，满足结束条件则需要退出循环
            if utterance_count & 1:  # utterance_count为奇数，ds模型生成话语阶段
                ...
                bspn_decoder_input_ids = self.tensorize([[self.dialog_tokenizer.pad_token_id] + [
                    self.dialog_tokenizer.convert_tokens_to_ids(definitions.BOS_BELIEF_TOKEN)]])
                ...
                if with_logprob:
                    ...
                    belief_states_prob = torch.max(torch.stack(model_output.scores, dim=1).softmax(-1), dim=-1).values[
                        0]
                else:
                ...
                dbpn_gen = self.encode_text(db_token, self.dialog_tokenizer, bos_token=definitions.BOS_DB_TOKEN,
                ...
                # goal_state_span = convert_goal_dict_to_span(goal_state_dict)  # 获取goal state
                ...
                goal_state_ids = self.encode_text(goal_state, self.simulator_tokenizer,
                                                  bos_token=definitions.BOS_GOAL_TOEKN,
                                                  eos_token=definitions.EOS_GOAL_TOKEN)
                ...
                if with_logprob:
                    ...
                    user_utterance_prob = torch.max(torch.stack(model_output.scores, dim=1).softmax(-1), dim=-1).values[
                        0]
                else:
                    user_utterance_output = model_output.cpu().numpy().tolist()
                    user_utterance_prob = None

                user_act, user_utterance, user_act_prob, _ = split_user_act_and_resp(self.simulator_tokenizer,
                                                                                     user_utterance_output[0])
    ...
    def generate_single_dialog(self, user_goal, with_logprob=False, agent=None):
        ...
        while is_continue(dial_gen):  # 需要判断一个会话是否结束，满足结束条件则需要退出循环
            if utterance_count & 1 :  # utterance_count为奇数，ds模型生成话语阶段
                ...
                goal_state_span = convert_goal_dict_to_span(goal_state_dict)  # 获取goal state
                goal_state_ids = self.encode_text(goal_state_span, self.simulator_tokenizer, bos_token=definitions.BOS_GOAL_TOEKN, eos_token=definitions.EOS_GOAL_TOKEN)  # 编码goal state
                encoded_dialog_history = [self.encode_text(text, self.simulator_tokenizer) for text in dialog_history]  # 编码对话历史
                context = self.flatten_dial_history(encoded_dialog_history, len(goal_state_ids), self.simulator_tokenizer.model_max_length)  # flatten并截断对话历史
                input_ids = self.tensorize([context + goal_state_ids + [self.simulator_tokenizer.eos_token_id]])  # input: dialog history + goal state span;
                input_ids = input_ids.to(device)
