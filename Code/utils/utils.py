...
def split_user_resp(tokenizer, model_output):
    if model_output[0] == tokenizer.pad_token_id:
        model_output = model_output[1:]
    if model_output[-1] == tokenizer.eos_token_id:
        model_output = model_output[:-1]
...
def split_system_resp(tokenizer, model_output):
...
# def split_system_resp(tokenizer, model_output, model_output_prob=None):
#     bos_resp_token_id = tokenizer.convert_tokens_to_ids(definitions.BOS_RESP_TOKEN)
#     eos_resp_token_id = tokenizer.convert_tokens_to_ids(definitions.EOS_RESP_TOKEN)
#
#     eos_token_id = tokenizer.eos_token_id
...
def calculate_bleu(results, agent_type):
    ...
    for id, dial in results.items():
        for turn in dial:
            count_turns += 1
            if agent_type == 'us':
                score_bleu4 += sentence_bleu([turn['user'].split()[1:-1]], turn['user_gen'].split(), smoothing_function=smoother.method0)
            elif agent_type == 'ds':
                score_bleu4 += sentence_bleu([turn['resp'].split()[1:-1]], turn['sys_gen'].split(), smoothing_function=smoother.method0)
    ...
    return score_bleu4 / count_turns * 100
...
if __name__ == '__main__':
    ...
    for idx in range(10):
        ...
        calculate_bert_score(file1, file2)
